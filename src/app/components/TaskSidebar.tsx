...
export default function TaskSidebar({ onAddTask }: { onAddTask: () => void }) {
  ...
  return (
    <section className="w-full lg:w-[26rem] h-auto lg:h-full glass-celestial border-t lg:border-t-0 lg:border-l border-outline-variant p-6 lg:p-8 overflow-y-auto z-10 flex flex-col relative shadow-celestial">
      ...
      {/* Progress Metric - Now outside the header div to match SSR expectations */}
      <div className="mb-10 p-5 rounded-2xl bg-surface-container-low border border-white/5 shrink-0">
        <div className="flex justify-between items-end mb-3">
          <span className="text-label-md-caps text-on-surface-variant">Celestial Progress</span>
          <span className="text-xl font-headline text-primary">{mounted ? completedTasks.length : 0} <small className="text-xs text-on-surface-variant opacity-60">/ {tasks.length} ARCS</small></span>
        </div>
        <div className="h-1.5 w-full bg-surface-container-highest rounded-full overflow-hidden text-[0px]">
          <div 
      ...
      <div className="space-y-6 flex-grow overflow-y-auto pr-2 pb-24">
        ...
        <div className="pt-6 border-t border-outline-variant">
          ...
          >
            <span className="text-label-md-caps">Archive ({mounted ? completedTasks.length : 0} Completed Today)</span>
