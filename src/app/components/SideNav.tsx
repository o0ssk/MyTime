...
export default function SideNav({ onAddTask }: { onAddTask: () => void }) {
  return (
    <aside className="fixed left-0 top-0 h-full hidden lg:flex flex-col py-12 px-6 z-40 glass-celestial w-72 rounded-r-xl border-none shadow-celestial font-body">
      <div className="mt-16 mb-12 flex flex-col items-center text-center">
        ...
        <p className="text-on-surface-variant text-label-md-caps mt-1">Time is a Field</p>
      </div>
      ...
      <div className="mb-8">
        <button onClick={onAddTask} className="w-full metallic-silver-gradient text-on-primary font-bold py-4 rounded-full shadow-silver-glow transition-transform active:scale-95 text-label-md-caps">
          Add Task Arc
        </button>
      </div>
